...
neo_dentiq.banner.render = function (frm, patient_field) {
	...
	frappe.call({
		...
		callback(r) {
			...
			const chips = `
				<span class="ndq-chip ${data.caries_risk === "Low" ? "good" : "warn"}">
					${__("Caries risk")}: ${data.caries_risk}</span>
				<span class="ndq-chip ${data.perio_risk === "Low" ? "good" : "warn"}">
					${__("Perio risk")}: ${data.perio_risk}</span>
				<span class="ndq-chip ${(data.no_show_score || 0) >= 50 ? "warn" : ""}">
					${__("No-show")}: ${data.no_show_score || 0}%</span>
