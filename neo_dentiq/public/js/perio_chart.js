...
class PerioGrid {
	...
	apply_band_colours() {
		// Perio band colours are defined in Neo Dentiq Theme, not hard-coded here.
		const root = document.documentElement;
		["healthy", "moderate", "deep", "bleeding"].forEach((band) => {
			const colour = neo_dentiq.theme.colour("perio", band, null);
			if (colour) root.style.setProperty(`--ndq-perio-${band}`, colour);
		});
	}
	...
	site_cells(row, pdFields, bopFields) {
		const coloured = neo_dentiq.theme.enabled("colour_coded_perio");
		return pdFields.map((field, i) => {
			const value = row[field] || 0;
			const cls = !coloured ? ""
				: value >= 6 ? "deep" : value >= 4 ? "moderate" : value > 0 ? "healthy" : "";
	...
	recolour($el) {
		...
		$cell.removeClass("deep moderate healthy");
		if (!neo_dentiq.theme.enabled("colour_coded_perio")) return;
		if (value >= 6) $cell.addClass("deep");
		else if (value >= 4) $cell.addClass("moderate");
		else if (value > 0) $cell.addClass("healthy");
	}
