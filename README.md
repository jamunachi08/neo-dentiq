...
ERPNext is a hard requirement. Compatible versions are declared in `pyproject.toml`:
...
2. Open **Neo Dentiq Theme** and pick a preset, or build your own palette.
3. Click **Install demo clinic** to create starter operatories, a weekday schedule, an
   autoclave and three instrument trays per kit.
4. Create your **Dental Practitioner** records and link each to an ERPNext Employee and a
   User.
...
Dentistry is a visual, reassuring speciality, and the interface is part of the chairside
experience. **Neo Dentiq Theme** is a settings page that colours the whole app — and every
part of it can be switched off.

### Presets

Six complete palettes ship with the app: **Clinical Mint** (teal and blue, the default),
**Coral Smile** (rose and orange), **Sunrise Ortho** (amber and pink), **Deep Ocean**
(blue and violet), **Berry Bright** (violet and cyan), and **Midnight Clinic** (a dark
palette for evening shifts and surgical suites). Editing any core colour switches the
preset to **Custom** automatically.

### Palette

Twelve colour tokens: primary, secondary, accent, success, warning, danger, info, page
background, card surface, text, muted text and border. Each becomes a CSS custom property
(`--ndq-primary` and friends) plus a soft tinted variant used for washes and pills, and a
three-stop gradient built from primary → secondary → accent.

### Shape, density and type

Corner radius (Sharp / Soft / Rounded / Pill), density (Comfortable / Compact), colour
intensity (Subtle / Balanced / Vivid), colour mode (Follow System / Light / Dark), and a
typeface picker (Inter, Nunito, Poppins, Source Sans 3, or the system stack). Web fonts
load only when a non-system face is selected.

### Feature switches

Twelve independent on/off toggles. Each maps to a body class, so turning one off genuinely
removes the styling rather than hiding it:

| Switch | Effect |
|---|---|
| Colourful sidebar & workspace icons | Tinted selection, coloured module icons |
| Gradient page headers | Gradient title text and a gradient underline |
| Coloured status pills | Themed indicators across every list view |
| Coloured workspace cards | Accent top-border per widget type |
| Colour-coded odontogram | Condition colour on tooth surfaces |
| Colour-coded perio grid | Depth-band shading on probing sites |
| Colour-coded appointment calendar | Status colours and a risk stripe in the diary |
| Colourful patient alert banner | Gradient banner, pulsing critical allergy pill |
| Animated transitions | Surface and pill transitions |
| Respect "Reduce Motion" | Honours the operating-system accessibility setting |
| Tooth glyphs on charts | Letter codes on treated surfaces |
| Colour-blind safe mode | Condition letters and pattern marks so colour is never the only signal |

### Editable colour maps

Six tables, each row with a key, label, colour, text colour, icon and its own **Active**
checkbox:

- **Tooth conditions** — 11 rows (Sound, Caries, Restored, Root Canal Treated, Crowned,
  Veneered, Fractured, Abraded, Discoloured, Mobile, Watch)
- **Tooth statuses** — 8 rows (Present, Missing, Extracted, Unerupted, Impacted, Implant,
  Pontic, Primary Retained)
- **Appointment statuses** — 10 rows driving the diary and every list view
- **Probing depth bands** — healthy, moderate, deep, bleeding
- **Role accents** — one colour per role
- **Module accents** — one colour per workspace

Deactivate a row and that condition stops being colour-coded; recolour a row and the
odontogram, legend and perio grid all follow on the next render. Nothing is hard-coded in
the charts.

### How it applies

The theme rides in on `boot_session`, so there is no flash of unstyled desk. `theme.js`
writes the variables onto `:root`, toggles the feature classes on `<body>`, and exposes
`neo_dentiq.theme.colour()` for the charts. Saving the theme publishes a realtime event, so
open sessions restyle without a reload. The payload is cached and served to the patient
portal too, so the public booking pages match the clinic's colours.

**Live preview** on the settings page renders the gradient hero, every swatch, the
appointment-status pills, the tooth-condition pills and a sample alert card as you edit.

A **Custom CSS** field is injected last for anything the switches do not cover, and
`Enable Neo Dentiq Theme` turns the entire layer off and returns the app to the stock
Frappe appearance.
